...
namespace DataLayer.Tests {
    ...
    public class UnitTest1 {
        ...
        public void ConsultaCatalogo() {
            ...
            int iMax = (pageNumber * pageSize) + 1;
            int iMin = ((pageNumber-1) * pageSize) + 1;

            String envolturaSelect = "SELECT * FROM (SELECT a.*, rownum rn FROM ( ";
            envolturaSelect += select;
            envolturaSelect += " ) a WHERE rownum < {0}) WHERE rn >= {1}";

            String selectFinal = String.Format(envolturaSelect, iMax, iMin);

            //String select2 = "select * from F55V137C limit 20";

            DataSet ds = ALC.IES.WebRange.DataLayer.Consultas.Execute(selectFinal);
